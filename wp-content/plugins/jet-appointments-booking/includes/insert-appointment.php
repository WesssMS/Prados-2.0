	...
	public function appointment_available( $appointments ) {
		...
		foreach ( $appointments as $appointment ) {
			...
			if ( ! Plugin::instance()->db->appointment_available( $appointment ) ) {
				$this->error_message = __( 'Appointment time already taken', 'jet-appointments-booking' );
				$notification_log = false;
				break;
			}

			if ( ! Time_Types::is_allowed_time( $appointment ) ) {
				$this->error_message = __( 'Selected time is not allowed to book', 'jet-appointments-booking' );
				$notification_log = false;
				break;
			}
		}
